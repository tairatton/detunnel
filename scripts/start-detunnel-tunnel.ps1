...
if ([string]::IsNullOrWhiteSpace($LnwjudPath)) {
  $LnwjudPath = if ($env:DETUNNEL_PATH) { $env:DETUNNEL_PATH } else { Join-Path $env:LOCALAPPDATA 'Programs\lnwjud\lnwjud.exe' }
}
...
try {
  ...
  Write-Host 'lnwjud tunnel: auto-restart is ON (TTL/exit 0 still restarts). Ctrl+C or DETUNNEL_TUNNEL_STOP=1 to stop.'

...
async function runBenchmark(options) {
  ...
  try {
    const [{ createStdioMcpRuntime }, { startMcpHttp }, { SqliteDatabase, SqliteWorkspaceRepository }, { WorkspaceService }] = await Promise.all([
      ...
      import('@detunnel/mcp-server'),
      import('@detunnel/storage'),
      import('@detunnel/workspace'),
    ]);

    const database = new SqliteDatabase(path.join(fixture.dataPath, 'detunnel.sqlite'));
...
async function createFixture() {
  const rootPath = await mkdtemp(path.join(os.tmpdir(), 'detunnel-baseline-'));
  const workspacePath = path.join(rootPath, 'workspace');
  const dataPath = path.join(rootPath, 'data');
  const files = {
    'package.json': JSON.stringify({
      name: 'detunnel-baseline-fixture',
      private: true,
      scripts: { test: 'node -e "process.exit(1)"' },
    }, null, 2) + '\n',
    'README.md': '# Baseline fixture\n\nThis workspace contains a small login flow for deterministic MCP benchmarks.\n',
    'src/auth/login.ts': [
...
function buildScenarios(workspaceId) {
  ...
  return [
    ...
    },
    {
      ...
      purpose: 'Search, inspect the suspected file, and correlate it with related tests.',
      steps: [
    ...
    {
      name: 'code-review',
      purpose: 'Review source structure and related symbols.',
      steps: [
        { tool: 'search_text', arguments: { ...common, query: 'export', glob: '*.{ts,tsx}', maxResults: 50 } },
        { tool: 'search_files', arguments: { ...common, glob: '*.{ts,tsx}', maxResults: 50 } },
      ],
    },
    {
      name: 'ui-debugging',
    ...
    },
    {
      ...
      purpose: 'Inspect test configuration, test matches, and the failing test surface.',
      steps: [
...
class McpHttpClient {
  ...
  async discover() {
    const initialize = await this.request('initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'detunnel-baseline-benchmark', version: '1.0.0' },
    });

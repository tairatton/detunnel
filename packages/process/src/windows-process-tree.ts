...
const TASKKILL_ALREADY_EXITED = 128;
...
export class WindowsProcessTree implements ProcessTreeTerminator {
  ...
  public async stop(child: ChildProcess, pid: number): Promise<void> {
    ...
    // taskkill can race with a process that exits naturally. In that case Windows
    // may return 128 before the root has reported its close event. Wait for the
    // root to settle within the bounded verifier window before accepting the
    // narrow race; other non-zero results remain unverified.
    if (taskkillExitCode !== 0) {
      if (taskkillExitCode === TASKKILL_ALREADY_EXITED) {
        try {
          await this.waitForExit(child);
        } catch {
          throw new Error(`Process tree termination exited with code ${taskkillExitCode}`);
        }
        if (child.exitCode !== null || child.signalCode !== null) {

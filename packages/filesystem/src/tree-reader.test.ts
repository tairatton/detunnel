...
describe('TreeReader', () => {
  it('sorts entries without hiding generated, hidden, dependency, or environment paths', async () => {
    ...
    await writeFile(path.join(root, '.source-metadata', 'config'), 'config', 'utf8');
    ...
    expect(result.value.entries.map((entry) => entry.path.replace(/\\/g, '/'))).toEqual(expect.arrayContaining([
      '.env', '.source-metadata', '.source-metadata/config', 'a.txt', 'dist', 'dist/app.js', 'node_modules', 'node_modules/hidden.txt', 'src', 'z.txt',
    ]));

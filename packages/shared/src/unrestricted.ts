...
export function unrestrictedFromEnv(env: ProcessEnvLike = process.env): boolean {
  return (parseFlag(env.DETUNNEL_UNRESTRICTED) ?? parseFlag(env.DETUNNEL_UNRESTRICTED)) === true;
}
...
export function isUnrestricted(env: ProcessEnvLike, settingValue: string | null | undefined): boolean {
  const fromEnv = parseFlag(env.DETUNNEL_UNRESTRICTED) ?? parseFlag(env.DETUNNEL_UNRESTRICTED);

...
const KEY_PREFIX_LOCAL_V1 = 'local:v1:';
...
export function loadCheckpointEncryptionKey(dataPath: string): Buffer {
  const configured = process.env.DETUNNEL_CHECKPOINT_KEY_BASE64;
  if (configured !== undefined && configured.trim().length > 0) {
    const key = Buffer.from(configured.trim(), 'base64');
    if (key.byteLength !== 32) throw new Error('DETUNNEL_CHECKPOINT_KEY_BASE64 must decode to 32 bytes');
    return key;
  }
  const filePath = path.join(dataPath, 'checkpoint-master.key');
  return process.platform === 'win32'
    ? loadOrCreateWindowsProtectedKey(filePath, 32)
    : loadOrCreateLocalProtectedKey(filePath, 32);
}

function loadOrCreateLocalProtectedKey(filePath: string, byteLength: number): Buffer {
  const absolutePath = path.resolve(filePath);
  mkdirSync(path.dirname(absolutePath), { recursive: true });
  try {
    const stored = readFileSync(absolutePath, 'utf8');
    chmodSync(absolutePath, 0o600);
    return decodeLocalProtectedKey(stored, byteLength);
  } catch (error) {
    if (!isMissingFile(error)) throw error;
  }

  const generated = randomBytes(byteLength);
  const protectedValue = KEY_PREFIX_LOCAL_V1 + generated.toString('base64');
  try {
    ...
    if (!isAlreadyExists(error)) throw error;
    chmodSync(absolutePath, 0o600);
    return decodeLocalProtectedKey(readFileSync(absolutePath, 'utf8'), byteLength);
  }
}
...
function decodeLocalProtectedKey(value: string, expectedLength: number): Buffer {
  const trimmed = value.trim();
  if (!trimmed.startsWith(KEY_PREFIX_LOCAL_V1)) throw new Error('Local protected key file has an unsupported format');
  const key = Buffer.from(trimmed.slice(KEY_PREFIX_LOCAL_V1.length), 'base64');
  if (key.byteLength !== expectedLength) throw new Error('Local protected key file has an invalid key length');
  return key;
}

...
function riskyCommandText(value: string): boolean {
  return /(?:^|[;&|]\s*|\b)(?:rm|del|erase|rmdir|rd|remove-item|clear-content|set-content|add-content|out-file|move-item|rename-item|copy-item|truncate|shred|dd|reg\s+(?:add|delete)|sc\s+(?:create|delete)|schtasks\s+\/(?:create|delete)|stop-process|stop-service|restart-service)\b/i.test(value);
}

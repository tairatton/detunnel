...
const QUERY_SCRIPT = [
  ...
  '$events = @()',
  'try {',
  '  if ($env:DETUNNEL_EVENT_MODE -eq \'crashes\') {',
  '    $since = (Get-Date).AddHours(-[double]$env:DETUNNEL_EVENT_HOURS)',
  '    $events = @(Get-WinEvent -FilterHashtable @{ LogName = \'Application\'; Id = @(1000, 1001); StartTime = $since } -MaxEvents ([int]$env:DETUNNEL_EVENT_MAX) -ErrorAction Stop)',
  '  } else {',
  '    $filter = @{}',
  '    if ($env:DETUNNEL_EVENT_LOG) { $filter.LogName = $env:DETUNNEL_EVENT_LOG }',
  '    if ($env:DETUNNEL_EVENT_PROVIDER) { $filter.ProviderName = $env:DETUNNEL_EVENT_PROVIDER }',
  '    if ($env:DETUNNEL_EVENT_SINCE) { $filter.StartTime = [datetime]::Parse($env:DETUNNEL_EVENT_SINCE) }',
  '    $events = @(Get-WinEvent -FilterHashtable $filter -MaxEvents ([int]$env:DETUNNEL_EVENT_MAX) -ErrorAction Stop)',
  '  }',
  '} catch {',
...
export class EventLogCapabilityBackend implements CapabilityBackend {
  ...
  public async execute(input: unknown, signal?: AbortSignal): Promise<Result<unknown>> {
    ...
    const environment: Record<string, string> = {
      DETUNNEL_EVENT_MODE: mode,
      DETUNNEL_EVENT_MAX: String(clampInteger(request.max_events ?? request.maxEvents, 100, 1, MAX_EVENTS_HARD_LIMIT)),
    };
    if (mode === 'crashes') {
      environment.DETUNNEL_EVENT_HOURS = String(clampNumber(request.hours, 24, 1, 720));
    } else {
      ...
      if (logName !== undefined) environment.DETUNNEL_EVENT_LOG = logName;
      if (provider !== undefined) environment.DETUNNEL_EVENT_PROVIDER = provider;
      ...
      if (since !== undefined) {
        ...
        environment.DETUNNEL_EVENT_SINCE = new Date(parsed).toISOString();
      }
    }

...
describe('EventLogCapabilityBackend', () => {
  it('runs an allowlisted query through the PowerShell runner with env-var parameters', async () => {
    ...
    expect(seen[0]!.environment).toMatchObject({ DETUNNEL_EVENT_MODE: 'query', DETUNNEL_EVENT_PROVIDER: 'Application Error', DETUNNEL_EVENT_LOG: 'Application', DETUNNEL_EVENT_MAX: '10' });
  ...
  it('maps crash_trace onto the bounded WER query', async () => {
    ...
    expect(seen[0]).toMatchObject({ DETUNNEL_EVENT_MODE: 'crashes', DETUNNEL_EVENT_HOURS: '12' });
    expect(seen[0]!.DETUNNEL_EVENT_PROVIDER).toBeUndefined();
  });
  ...
  it('bounds max_events and validates the since timestamp', async () => {
    ...
    expect(seen.at(-1)!.DETUNNEL_EVENT_MAX).toBe('500');

    await backend.execute({ operation: 'query', provider: '.NET Runtime', since: '2026-08-21T10:00:00Z' });
    expect(seen.at(-1)!.DETUNNEL_EVENT_SINCE).toBe('2026-08-21T10:00:00.000Z');

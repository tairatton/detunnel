...
function toolContract(): string {
  const filename = path.resolve(import.meta.dirname, '..', '..', '..', 'docs', 'architecture', 'TOOL_CONTRACT.md');
  try {
...
describe('mutation inventory completeness', () => {
  ...
  it('documents every advertised MCP tool in the generated tool contract', () => {
    const matrix = toolContract();

 ...
 * (DETUNNEL_LSP_<LANGUAGE>_COMMAND, JSON argv preferred). Workspace files are
...
export class LspRuntimeService {
  ...
  private async startSession(
    ...
    if (command === undefined) {
      return err(appError('PERMISSION_DENIED', `No language server configured for ${language}. Set DETUNNEL_LSP_${language.toUpperCase()}_COMMAND (JSON argv preferred)`));
    }
  ...
  private serverCommand(language: string): readonly string[] | undefined {
    ...
    const configured = readString(settingsCommand) ?? this.environment[`DETUNNEL_LSP_${language.toUpperCase()}_COMMAND`];

...
export const SCHEDULED_CONTINUATION_FENCED_TOOLS = new Set([
  ...
  'restore_deleted_file', 'restore_checkpoint', 'shell', 'wsl_exec',
  'process_start', 'process_stop', 'project_dev', 'project_test', 'project_lint', 'project_typecheck', 'project_build',
  'codex_run', 'codex_stop', 'agent_swarm_run', 'self_heal_apply',
...
function summarizeMutationForApproval(toolName: string, input: unknown, activeWorkspaceScope: WorkspaceScope | null): string {
  ...
  if (toolName === 'agent_swarm_run') {
    ...
    lines.push('WARNING: this consumes explicitly enabled Codex quota; v2.0.0 enforces read-only child sandboxes.');
    return boundedApprovalSummary(lines);
  }
...
const LOCAL_MUTATION_TOOLS = new Set(['write_file', 'apply_patch', 'edit_file', 'move_file', 'copy_file', 'delete_file', 'restore_deleted_file', 'restore_recovery_item', 'restore_checkpoint', 'shell', 'wsl_exec', 'process_start', 'process_stop', 'codex_run', 'codex_stop', 'agent_swarm_run', 'office', 'office_ppt', 'docx_merge', 'self_heal_apply']);

...
export interface ContextFile {
  ...
  readonly workspaceRelevance: 'matched' | 'related' | 'none';
...
interface Candidate {
  ...
  readonly reason: string;
  readonly workspaceRelevance: ContextFile['workspaceRelevance'];
...
export class ContextEngine {
  ...
  private async collectWorkspace(workspaceId: string, request: WorkspaceContextRequest): Promise<Result<WorkspaceCollection>> {
    ...
    const [textResult, filesResult] = await Promise.all([searchTextPromise, searchFilesPromise]);
    ...
    const ranked = [...candidates.values()]
      .filter((candidate) => {
        const allowed = request.includeIgnored === true || classifyContextPath(candidate.path, 'automatic').discoverable;
        if (!allowed) this.economy.recordSkipped(candidate.path);
        return allowed;
      })
      .map((candidate): Candidate => {
      const workspaceRelevance: ContextFile['workspaceRelevance'] = candidate.matches.length > 0 ? 'matched' : candidate.fromFilename ? 'related' : 'none';
      const testRelevance = isTestPath(candidate.path) ? 'test' : isSourcePath(candidate.path) ? 'source' : 'unknown';
      const score = candidate.score
        + (workspaceRelevance === 'matched' ? 20 : workspaceRelevance === 'related' ? 4 : 0)
      ...
      const reasons = [
        ...
        ...(workspaceRelevance === 'matched' ? ['workspace content match'] : workspaceRelevance === 'related' ? ['workspace path match'] : []),
    ...
    try {
      ...
      if (!result.ok) {
        return {
          ...
          snippets: [],
          symbols: [],
          workspaceRelevance: candidate.workspaceRelevance,
      ...
      const prepared = this.economy.prepare({
        ...
        contextId,
        discovery: request.includeIgnored === true || candidate.workspaceRelevance === 'matched' ? 'explicit' : 'automatic',
      });

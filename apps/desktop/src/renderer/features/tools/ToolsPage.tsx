  ...
  return (
    <section className="panel tools-page" aria-labelledby="tools-heading">
      ...
      <div className="tool-origin-tabs" role="tablist" aria-label={locale === 'th' ? 'แหล่งเครื่องมือ' : 'Tool origin'}>
        <button type="button" role="tab" aria-selected={origin === 'lnwjud'} className={origin === 'lnwjud' ? 'active' : undefined} onClick={() => setOrigin('lnwjud')}>detunnel ({items.filter((item) => item.origin === 'lnwjud').length})</button>

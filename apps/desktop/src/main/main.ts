...
// so the ESM desktop bundle works on Node/Electron versions that do not expose
// CommonJS properties as named ESM exports.
...
const defaultDestructiveDeletePolicy: DestructiveDeletePolicy = {
  ...
  approvals: { delete_file: false, shell_rm_unlink: false, shell_rmdir: false, shell_del_erase: false, wsl_rm_unlink: false, wsl_rmdir: false },
};
...
const emptyRemoteMcp: RemoteMcpStatus = {
  state: 'stopped', provider: 'cloudflared', installed: false, hasAuthtoken: false, providerPath: null,
...
export function registerIpcHandlers(
  ...
  const incidentSaver = new IncidentSaveCoordinator({
    ...
    choosePath: async (): Promise<string | null> => {
      ...
      const result = await dialog.showSaveDialog(window, { title: 'Capture detunnel incident evidence', defaultPath: 'detunnel-incident.json', filters: [{ name: 'JSON', extensions: ['json'] }] });
  ...
  ipcMain.handle(ipcChannels.chooseWorkspaceFolder, async (event, payload: unknown) => {
    assertTrustedSender(event, getMainWindow());
    assertNoPayload(payload);
    const window = getMainWindow();
    if (window === null) return { rootPath: null };
    const result = await dialog.showOpenDialog(window, {
      title: 'Select project folder',
      properties: ['openDirectory', 'createDirectory'],
    });
    return { rootPath: result.canceled ? null : (result.filePaths[0] ?? null) };
  });
...
function parseAddWorkspaceRequest(payload: unknown): AddWorkspaceRequest {
  ...
  if (payload.makePrimary !== undefined && typeof payload.makePrimary !== 'boolean') throw new Error('Invalid makePrimary');
  return { rootPath: nonEmptyString(payload.rootPath, 'rootPath'), makePrimary: payload.makePrimary === true };
}
...
async function exportLogsToFile(
  ...
  const result = await dialog.showSaveDialog(window, {
    title: 'Export detunnel logs',
    defaultPath: `detunnel-${request.source}-logs.txt`,
...
async function exportWorkLogToFile(window: BrowserWindow | null, services: DesktopIpcServices, request: ExportWorkLogRequest): Promise<{ readonly exported: boolean }> {
  if (window === null) return { exported: false };
  const result = await dialog.showSaveDialog(window, {
    title: 'Export detunnel work log',
    defaultPath: 'detunnel-work-log.txt',
    filters: [{ name: 'Text', extensions: ['txt', 'log'] }],
  });
...
function handleDesktopStartupFailure(scope: string, error: unknown): void {
  ...
  try {
    dialog.showErrorBox('detunnel failed to start', scope + ' startup failed.\n\n' + message);
  } catch {
...
function configureDesktopShutdown(runtime: DesktopRuntime): void {
  desktopShutdownCoordinator = new DesktopShutdownCoordinator({
    ...
    onDeferred: (error): void => {
      ...
      void dialog.showMessageBox({
        type: 'error',
        title: 'detunnel is still running',
        message: 'The owned tunnel could not be confirmed stopped. detunnel will remain open; retry Quit after checking the tunnel status.',

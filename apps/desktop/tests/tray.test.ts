...
describe('desktop tray behavior', () => {
  ...
  it('localizes updater state labels and the tray tooltip', () => {
    ...
    expect(createTrayToolTip('th')).toBe('detunnel — ทำงานเบื้องหลัง');
    expect(createTrayToolTip('en')).toBe('detunnel — running in background');
  });
  ...
  it('keeps manual update feedback localized in the native catalog', () => {
    expect(nativeMessages('th').updateCurrentDialog('4.6.1')).toBe('detunnel v4.6.1 เป็นเวอร์ชันล่าสุดแล้ว');
    expect(nativeMessages('en').updateCurrentDialog('4.6.1')).toBe('detunnel v4.6.1 is up to date');
  });
});

  ...
  it('keeps the bundled production MCP stdio entrypoint alive across sequential calls and an error on a non-E workspace', async () => {
    ...
    const transport = new StdioClientTransport({
      ...
      env: {
        ...
        DETUNNEL_RESET_WORKSPACES: '1',
        DETUNNEL_CONFIRM_RESET_WORKSPACES: 'DELETE-REGISTERED-WORKSPACES',
...
async function currentOwner(): Promise<TunnelLockOwner> {
  const result = await runPowerShell("$p = Get-CimInstance Win32_Process -Filter \"ProcessId = $env:DETUNNEL_ACCEPTANCE_PID\"; $p.CreationDate.ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ss.fffZ', [Globalization.CultureInfo]::InvariantCulture)", { DETUNNEL_ACCEPTANCE_PID: String(process.pid) });

...
describe('detunnel tunnel ownership lock', () => {
  ...
  it('publishes no partial fixed record and gives a delayed writer no second ownership', async () => {
    ...
    await expect(access(path.join(directory, 'detunnel.tunnel.lock'))).rejects.toThrow();
  ...
  it('reclaims a lock only after the recorded owner is gone or has a mismatched start time', async () => {
    ...
    await writeFile(path.join(directory, 'detunnel.tunnel.lock'), JSON.stringify({ version: 1, ...staleOwner }), 'utf8');
  ...
  it('reclaims a lock when the recorded owner process is gone', async () => {
    const directory = await temporaryDirectory();
    await writeFile(path.join(directory, 'detunnel.tunnel.lock'), JSON.stringify({ version: 1, ...owner(707, '2026-08-20T00:00:00.000Z') }), 'utf8');
  ...
  it('only releases a lock that still belongs to its owner', async () => {
    ...
    await writeFile(path.join(directory, 'detunnel.tunnel.lock'), JSON.stringify({ version: 1, ...replacement }), 'utf8');
  ...
  it('restores a replacement moved by an owner-release race instead of deleting it', async () => {
    ...
    await rename(path.join(directory, 'detunnel.tunnel.lock'), path.join(directory, 'original-owner-record'));
    await writeFile(path.join(directory, 'detunnel.tunnel.lock'), JSON.stringify({ version: 1, ...replacement }), 'utf8');
  ...
  it('never guesses ownership from an invalid fixed lock record', async () => {
    const directory = await temporaryDirectory();
    const lockPath = path.join(directory, 'detunnel.tunnel.lock');
  ...
  it('serializes stale replacement so two reclaimers and a third publisher cannot fill the fixed-path gap', async () => {
    ...
    await writeFile(path.join(directory, 'detunnel.tunnel.lock'), JSON.stringify({ version: 1, ...stale }), 'utf8');
...
async function temporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'detunnel-tunnel-lock-'));

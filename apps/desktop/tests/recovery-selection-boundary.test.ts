...
describe('Recovery Center selected-workspace authority', () => {
  it('shows only recovery entries belonging to the host-selected workspace', async () => {
    const rawData = await mkdtemp(path.join(os.tmpdir(), 'detunnel-recovery-selection-data-'));
    const rawA = await mkdtemp(path.join(os.tmpdir(), 'detunnel-recovery-selection-a-'));
    const rawB = await mkdtemp(path.join(os.tmpdir(), 'detunnel-recovery-selection-b-'));

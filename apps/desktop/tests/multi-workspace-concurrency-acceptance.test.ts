...
describe('multi-workspace concurrency acceptance', () => {
  it('runs two real MCP sessions in parallel without mixing workspace or session ownership', async () => {
    ...
    try {
      ...
      const primaryBeforeAdd = (await runtime.services.getDashboard()).selectedWorkspace?.id;
      const workspaceRootC = await createWorkspaceFixture('c');
      const workspaceC = await runtime.services.addWorkspace({ rootPath: workspaceRootC });
      expect((await runtime.services.getDashboard()).selectedWorkspace?.id).toBe(primaryBeforeAdd);
      const liveAddedProject = await clientA.callTool({
        name: 'write_file',
        arguments: { workspaceId: workspaceC.id, path: 'live-added.txt', content: 'available without restart' },
      });
      expect(liveAddedProject.isError).not.toBe(true);
      const sameWorkspace = await runtime.services.addWorkspace({ rootPath: workspaceRootC, makePrimary: true });
      expect(sameWorkspace.id).toBe(workspaceC.id);
      expect((await runtime.services.getDashboard()).selectedWorkspace?.id).toBe(workspaceC.id);
      await runtime.services.selectWorkspace({ workspaceId: primaryBeforeAdd! });
      await runtime.services.setWorkspaceActive({ workspaceId: workspaceC.id, active: false });

import { appError, ok } from '@detunnel/domain';
import type { McpServerOptions } from '@detunnel/mcp-server';
  ...
  it('resolves the configured workspace before starting the server', async () => {
    ...
    const result = await runMcpStdioCommand({
      ...
      createServerOptions: (selectedWorkspace) => ({
        services: {},
        actor: { clientId: selectedWorkspace.id, clientName: 'detunnel-cli' },
      }),
      starter,
    });

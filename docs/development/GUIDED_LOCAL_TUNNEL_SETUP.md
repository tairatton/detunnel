...
detunnel includes a bilingual first-run guide for connecting ChatGPT through the official OpenAI Secure MCP Tunnel without requiring users to edit config files or run Terminal commands.

## Who sees the first-run Tips dialog

The dialog is shown only when the local tunnel setup is pristine: there is no saved Runtime API key, no configured tunnel profile, no persisted Tunnel ID, and the guide has not previously been dismissed or completed. Existing users with a key or profile are not interrupted automatically. If setup was started but not completed, detunnel resumes the Secure Tunnel guide from the state it can derive from the actual tunnel status.
...
1. เปิดหน้า OpenAI Tunnel Settings เลือกองค์กรที่ใช้กับ ChatGPT สร้าง tunnel แล้วคัดลอกค่า `tunnel_...` มาวางใน detunnel หากสร้างไม่ได้ ให้ตรวจสิทธิ์ Tunnels Read + Manage.
2. เปิดหน้า API Keys สร้าง Runtime API key ที่มีสิทธิ์ Tunnels Read + Use แล้ววางใน detunnel. คีย์เต็มจะแสดงจาก OpenAI เพียงครั้งเดียว.
3. กด **ตั้งค่าและตรวจสอบ** ให้ detunnel ใช้ `tunnel-client` ที่มากับโปรแกรมเพื่อสร้าง local profile และตรวจสอบการเชื่อมต่อ.
4. กด **Start Tunnel**. detunnel จะถือว่าขั้นตอนฝั่งเครื่องเสร็จเมื่อสถานะจริงกลับมาเป็น Running เท่านั้น และจะ reconnect Tunnel ID เดิมอัตโนมัติเมื่อจำเป็น.
...
1. Open OpenAI Tunnel Settings, choose the organization used with ChatGPT, create a tunnel, and paste the `tunnel_...` value into detunnel. If tunnel creation is unavailable, verify Tunnels Read + Manage permissions.
2. Open API Keys, create a Runtime API key with Tunnels Read + Use, and paste it into detunnel. OpenAI displays the full secret only once.
3. Select **Configure and check**. detunnel uses the bundled `tunnel-client` to create the local profile and validate the connection.
4. Select **Start Tunnel**. Local setup is considered complete only after the real status returns Running. detunnel reconnects the same Tunnel ID automatically when required.
...
The Runtime API key is passed to the existing desktop credential path only when the user saves it. On Windows it is protected with DPAPI and stored locally. detunnel does not operate a central credential server. The onboarding state stored in renderer storage is only one finite UI value: `not_started`, `in_progress`, `dismissed`, or `completed`.
...
## What detunnel can and cannot prove

detunnel can verify the local configuration and observe the tunnel reaching **Running**. It cannot verify that the ChatGPT plugin connection was successfully added inside ChatGPT, because that final connection step is performed by the user in ChatGPT and is outside the local application's authority.
